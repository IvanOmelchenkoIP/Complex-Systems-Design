...
public class SelectThread2 implements Runnable {
	...
	public SelectThread2(String url, SQLConnectionManager sqlConnectionManager, int loops, Lock lock,
	...
	public void run() {
		...
		try {
			...
			System.out.println("Помилка при встановленні з\'єднання з базою даних у потоці SELECT 2...");
		...
		try {
			...
			System.out.println("Помилка у роботі потоку SELECT 2...");
			System.out.println(e);
			return;
		} finally {
			countDownLatch.countDown();
	...
	private void closeConnections(Connection connection, Statement statement) {
		try {
			...
			System.out.println("Помилка при закритті з\'єднання з базою даних у потоці SELECT 2...");
			System.out.println(e);
		}
	}
}
