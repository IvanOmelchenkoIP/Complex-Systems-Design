...
public class SelectThread3 implements Runnable {
	...
	public SelectThread3(String url, SQLConnectionManager sqlConnectionManager, int loops, Lock lock,
	...
	public void run() {
		...
		try {
			...
			System.out.println("Помилка при встановленні з\'єднання з базою даних у потоці SELECT 3...");
		...
		try {
			...
			System.out.println("Помилка у роботі потоку SELECT 3...");
			System.out.println(e);
			return;
		} finally {
			countDownLatch.countDown();
	...
	private void closeConnections(Connection connection, Statement statement) {
		try {
			...
			System.out.println("Помилка при закритті з\'єднання з базою даних у потоці SELECT 3...");
			System.out.println(e);
		}
	}
}
